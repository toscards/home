...
if ($_GET['typ'] == 'del'){
	$delete ="DELETE from projects WHERE ProjectId=".$_GET['id']."";
	mysqli_query($conn,$delete);//  or die(mysql_error());
	mysqli_close($conn);
	header('LOCATION: projectsdit.php?msg=Report Deleted Successfully!');
	exit;
}
?><!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>Manage Projects - Faculty Management System</title>
    ...
    <div class="container">
     ...
     <div class="page-header" id="banner">
        <div class="row">
		  ...
		  <?
		  $sql=" SELECT * from projects pp, departments d, semesters se, classes c WHERE  pp.DeptId = d.DeptId AND pp.SemesterId = se.SemesterId AND pp.ClassId = c.ClassId ORDER BY ProjectTitle ASC";
			//print_r($sql);
...
?>
 ...
 //Date into Readable format
 	?>
  <tr>
      <td>&nbsp;<?=$ListData['ProjectId'];?></td>
      <td><?=$ListData['ProjectTitle'];?></td>
      <td><?=$ListData['DeptInitials'];?></td>
      <td><?=$ListData['ClassInitials'];?></td>
      <td><?=$ListData['SemesterName'];?></td>
      <td><a href="projects/<?=$ListData['ProjectFileName'];?>" target="_blank"> View Project</a></td>
     <td><a href="projectsdit.php?id=<?=$ListData['ProjectId'];?>&typ=edit" ><b>Edit Details</b></a></td>
	   <td><a href="projectsdit.php?id=<?=$ListData['ProjectId'];?>&typ=del" onclick="return confirm('Are you sure you want to Delete this Project?');" ><b>Delete Project</b></a></td>
...
$GetReport = "SELECT * from projects where ProjectId=".$_GET['id'];
...
<?    $i= $i+1; }  }else{ echo "No Streams added to Display"; } mysqli_close($conn); ?>
	 ...
	 <div class="form-group">
      <label for="inputPassword" class="col-lg-2 control-label">Project Title</label>
      <div class="col-lg-10">
        <input class="form-control" id="inputPassword" placeholder="Enter projec ttitle" type="text" name="projecttitle" value="<?=$FetchReport['ProjectTitle'];?>" maxlength="100" required>
         </div>
    </div>
	...
	<div class="form-group">
      ...
      <div class="col-lg-10">
			Current File: <a href="project/<?=$FetchReport['ProjectFileName'];?>" target="_blank"> View Report</a><br><br>
    ...
    <div class="form-group">
	  ...
	  <input type="hidden" value="projectupdate" name="formtype"/>
	  <input type="hidden" value="<?=$_GET['id'];?>"  name="projectid"/>
	  <input type="hidden" value="<?=$FetchReport['ProjectFileName'];?>"  name="projectfilename"/>
